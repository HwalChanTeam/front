...
function BuyInfo(props) {
    ...
    const getPaymentNum = async (orderId) => {
        let response = null;
        try {
            response = await instance.get("/user/payment", { params: { orderId: orderId } });
            console.log(response);
    ...
    const accessTokenMutaion = useMutation(
        async () => await axios.post("https://api.portone.io/login/api-secret", 
            { "apiSecret": portone },
        ),
        {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                accessTokenMutaion.mutateAsync().then(response => 
                    portonePaymentCancelMutation.mutateAsync(response?.data).catch(() => {}),
                );
            }
        });
    };
